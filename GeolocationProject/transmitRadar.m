function [rxSig_] = transmitRadar (sig, chanModel, fs, fc, pPos,  pVel, pUEPos,  pUEVel, isChannel)
...
if (isequal(chanModel,'freespace') ||  isequal(chanModel,'two-ray'))
    ...
    [UErngTx,UEangTx]       = rangeangle(bPos, pUEPos, chanModel);  % second input is the reference
    [UErngRx,UEangRx]       = rangeangle(pUEPos, bPos, chanModel);
else
    [bsrngTx, bsangTx] = rangeangle( bPos, pPos);
    [bsrngRx, bsangRx] = rangeangle( pPos, bPos);

    % 5G transmit and receive directions
    [UErngTx,UEangTx]       = rangeangle(bPos, pUEPos);  % second input is the reference
    [UErngRx,UEangRx]       = rangeangle(pUEPos, bPos);
end
...
if isChannel
    ...
    if (isequal(chanModel,'freespace') ||  isequal(chanModel,'two-ray'))
        ...
        txsigUE        = channel(txsigUE,pUEPos,bPos,pUEVel,bVel);
    else
        [txsig,~]     = channel( txsig);
...
rxcol          = collector([txsig txsigUE],[bsangRx UEangRx]);
