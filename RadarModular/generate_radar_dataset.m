% Generate radar Dataset, SEC Full, SEC sparse and save it to folders according to Sklrk
function [data, rP] = generate_radar_dataset( inputPathDirIQ, outputPathDirIQ, mode, rP)

% For generating radar signal, channel parameters, snr are taken from params.m
% Generate radar signal, apply channel + snr (rxsig)
...
% 0-bit (1)  represents generate radar signal, rxsig
% 1-bit (2)  represents write radar signal to file
...
if rP.debugLog
    fprintf(' Start Radar Dataset Generating Process... \n');
end
% Write to results
fprintf(rP.fileID,' Start Radar Dataset Generating Process... \n');
...
if bitand(mode , 1)
    ...
    for indSnr = 1: Nsnr
        curSNR = SNR(indSnr);
        for w = 1 : NwaveF
            %Select the Radar Waveform
            curWaveF   = WAVEFORMS{w};   % LFM Waveform from the Radar Parameters
            ...
            if ispc
                ...
                NameSigMatFile = [curOutputDir,  '\radarSig.mat'];
                NameSECMatFile = [curOutputDir,  '\radarSEC.mat'];
            else
                curOutputDir = [outputPathDir, '/datasetSNR', int2str(SNR(indSnr)), gnbPart, '/' , curWaveF];
            ...
            %[sig, Np, PRI, Ns]
            [sig, ~, ~, ~] = generateSig (curWaveF, fs, Ts,  sigScaling);
            [rxSig_] = transmitRadar (sig, chanModel, fs, fc, rP.pPos,  rP.pVel, true);
            pRMS = rms(rxSig_(:))^2;
            ...
            if bitand( mode , 2 ) % Writing to .dat binary files
                for indCh = 1:length (radioCh)
                    ...
                    % Save received radar waveform in .dat file
...
elseif  bitand(mode , 32) % load IQ dataset
    ...
    if ispc
        ...
        NameSigMatFile = [outputPathDir,  '\radarSig.mat'];
...
elseif bitand(mode , 64) % load SEC dataset
    ...
    % Create the SEC, SECSparse folders as well as sig.mat, SEC.mat files
    if ispc
        NameSECMatFile = [outputDir,  '\radarIntSEC.mat'];
    else
        NameSECMatFile = [outputDir,  '/radarIntSEC.mat'];
    end
    ...
    [data, rP] = preprocessRadar(data, [], rP, 3);
...
[data, rP] = preprocessRadar(data, rxSigN, rP, 2);
